...
For auto-pipelined RAMs, `_register_pipeline_latency` records the callable's
`_auto_pipeline_ram` metadata in `ParserState.auto_pipeline_rams` alongside its
selected fixed latency. Each candidate is re-elaborated as ordinary fixed-latency
hardware; the shared backend aligns callers without slicing through memory state.
Live simulation roots generated by factories obtain their source from `linecache`.
See [auto-pipelined RAM compilation](AUTO_PIPELINE_DESIGN.md#ram-compiler-sweep-and-simulation).

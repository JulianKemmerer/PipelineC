...
def PART_SET_TOOL(part_str, allow_fail=False):
    ...
    if SYN_TOOL is None:
        ...
        if part_str is None:
            ...
            if part_str.lower().startswith("xc"):
            ...
            elif part_str.lower().startswith("ep") or part_str.lower(
            ).startswith("10c") or part_str.lower().startswith("5c"):
                SYN_TOOL = QUARTUS
            ...
            elif part_str.lower().startswith(
                    "lfe5u") or part_str.lower().startswith("ice"):
            ...
            elif part_str.upper().startswith(
                    "T8") or part_str.upper().startswith("TI"):
                SYN_TOOL = EFINITY
                ...
                if not allow_fail:
                    print("No known synthesis tool for FPGA part:",
                          part_str,
                          flush=True)
                    sys.exit(-1)

        if SYN_TOOL is not None:
            print("Using", SYN_TOOL.__name__, "synthesizing for part:",
                  part_str)
...
class MultiMainTimingParams:
    ...
    def GET_HASH_EXT(self, parser_state):
        ...
        s = top_level_str
        hash_ext = "_" + ((hashlib.md5(s.encode("utf-8")).hexdigest())[0:4]
                          )  #4 chars enough?
        return hash_ext
...
class TimingParams:
    ...
    def GET_PIPELINE_LOGIC_LATENCY(self, parser_state,
                                   TimingParamsLookupTable):
        total_latency = self.GET_TOTAL_LATENCY(parser_state,
                                               TimingParamsLookupTable)
    ...
    def CALC_TOTAL_LATENCY(self, parser_state, TimingParamsLookupTable=None):
        ...
        if len(self.logic.submodule_instances) <= 0:
            ...
            pipeline_map = GET_PIPELINE_MAP(self.inst_name, self.logic,
                                            parser_state,
                                            TimingParamsLookupTable)
    ...
    def RECURSIVE_GET_IO_REGS_AND_NO_SUBMODULE_SLICES(self, inst_name, Logic,
                                                      TimingParamsLookupTable,
                                                      parser_state):
        ...
        if len(Logic.submodule_instances) > 0:
            ...
            for submodule in sorted(Logic.submodule_instances
                ...
                rv += (self.RECURSIVE_GET_IO_REGS_AND_NO_SUBMODULE_SLICES(
                    sub_inst, sub_logic, TimingParamsLookupTable,
                    parser_state), )
        else:
            # Raw HDL
    ...
    def BUILD_HASH_EXT(self, inst_name, Logic, TimingParamsLookupTable,
                       parser_state):
    ...
    def GET_HASH_EXT(self, TimingParamsLookupTable, parser_state):
        if self.hash_ext is None:
            self.hash_ext = self.BUILD_HASH_EXT(self.inst_name, self.logic,
                                                TimingParamsLookupTable,
                                                parser_state)
    ...
    def ADD_SLICE(self, slice_point):
        ...
        if self.calcd_total_latency is not None:
            print("WTF adding a slice and has latency cache?",
                  self.calcd_total_latency)
            print(0 / 0)
            sys.exit(-1)
    ...
    def GET_SUBMODULE_LATENCY(self, submodule_inst_name, parser_state,
                              TimingParamsLookupTable):
    ...
    for logic_inst_name in parser_state.LogicInstLookupTable:
        ...
        if logic_i.func_name in _GET_ZERO_CLK_HASH_EXT_LOOKUP_cache:
            ...
            _GET_ZERO_CLK_HASH_EXT_LOOKUP_cache[
                logic_i.func_name] = timing_params_i.GET_HASH_EXT(
                    ZeroClockTimingParamsLookupTable, parser_state)

    _GET_ZERO_CLK_TIMING_PARAMS_LOOKUP_cache[
        cache_key] = ZeroClockTimingParamsLookupTable
...
def GET_ZERO_CLK_PIPELINE_MAP(inst_name,
                              Logic,
                              parser_state,
                              write_files=True):
    key = Logic.func_name
    ...
    ZeroClockLogicInst2TimingParams = GET_ZERO_CLK_TIMING_PARAMS_LOOKUP(
        parser_state)
    ...
    # Get pipeline map
    zero_clk_pipeline_map = GET_PIPELINE_MAP(inst_name, Logic, parser_state,
                                             ZeroClockLogicInst2TimingParams)
    ...
    if zero_clk_pipeline_map.zero_clk_max_delay is not None:
        ...
        # Sanity?
        if has_delay:
            print("has delay for subs but did not calc in zero clk map?",
...
class SubmoduleLevelInfo:
    ...
    def IS_EMPTY(self):
        return (len(self.driver_driven_wire_pairs) == 0
                and len(self.submodule_insts) == 0)
...
class PipelineMap:
    ...
    def __init__(self, logic):
        ...
        # HELP ME
        self.zero_clk_per_delay_submodules_map = dict(
        )  # dict[delay_offset] => [submodules,at,offset]
        self.zero_clk_submodule_start_offset = dict(
        )  # dict[submodule_inst] = start_offset  # In delay units
        self.zero_clk_submodule_end_offset = dict(
...
def GET_PIPELINE_MAP(inst_name, logic, parser_state, TimingParamsLookupTable):
    ...
    if VHDL.LOGIC_IS_RAW_HDL(
            logic
    ) or logic.is_vhdl_func or logic.is_vhdl_expr or logic.is_vhdl_text_module or logic.func_name in parser_state.func_marked_blackbox:
        return rv
            ...
            while len(wires_to_follow) > 0:
                ...
                for wire_to_follow in wires_to_follow:
                    if wire_to_follow in logic.wire_drives:
                        wire_to_follow_driven_wires = logic.wire_drives[
                            wire_to_follow]
    ...
    for wire_driven_so_far in list(wires_driven_by_so_far.keys()):
        delay_offset_when_driven[
            wire_driven_so_far] = 0  # Is per stage value but we know is start of stage 0
    ...
    def PIPELINE_DONE():
        ...
        for output in logic.outputs:
            if (output not in wires_driven_by_so_far) or (
                ...
                return False
            if print_debug:
                print("Output driven ", output, "<=",
                      wires_driven_by_so_far[output])
        # Feedback wires
        for feedback_var in logic.feedback_vars:
            if (feedback_var not in wires_driven_by_so_far) or (
                ...
                return False
            if print_debug:
                print("Feedback var driven ", feedback_var, "<=",
        ...
        for state_reg in logic.state_regs:
            if logic.state_regs[state_reg].is_volatile == False:
                ...
                if (global_wire not in wires_driven_by_so_far) or (
                    ...
                    return False
                if print_debug:
                    print("Global driven ", global_wire, "<=",
        ...
        for global_wire in logic.write_only_global_wires:
            if (global_wire not in wires_driven_by_so_far) or (
                ...
                return False
            if print_debug:
                print("Write only global driven ", global_wire, "<=",
                      wires_driven_by_so_far[global_wire])
        ...
        for state_reg in logic.state_regs:
            if logic.state_regs[state_reg].is_volatile:
                ...
                if volatile_global_wire in logic.wire_driven_by:
                    ...
                    if (volatile_global_wire not in wires_driven_by_so_far
                        ) or (wires_driven_by_so_far[volatile_global_wire]
                              == None):
                        if print_debug:
                            print("Pipeline not done volatile global.",
                                  volatile_global_wire)
                        return False
                if print_debug:
                    print("Volatile driven ", volatile_global_wire, "<=",
                ...
                for driving_wire in wires_to_follow:
                    ...
                    if driving_wire not in wires_driven_by_so_far:
                        ...
                        if driving_wire in logic.wire_driven_by:
                            driver_of_driver = logic.wire_driven_by[
                                driving_wire]
                    ...
                    if C_TO_LOGIC.WIRE_IS_SUBMODULE_PORT(
                            driving_wire, logic):  # is output checked next
                        sub_out_toks = driving_wire.split(
                        ...
                        if output_port in submodule_logic.outputs:
                            ...
                            submodule_latency_from_container_logic = timing_params.GET_SUBMODULE_LATENCY(
                                submodule_inst_name, parser_state,
                                TimingParamsLookupTable)
                            if submodule_latency_from_container_logic > 0:
                                stage_info.submodule_output_ports.append(
                                    driving_wire)
                    ...
                    if driving_wire in logic.wire_drives:
                        ...
                        for driven_wire in driven_wires:
                            ...
                            if not (driven_wire in logic.wire_driven_by):
                                # In
                                print("!!!!!!!!!!!! DANGLING LOGIC???????",
                                      driven_wire, "is not driven!?")
                                continue
                            ...
                            if (driven_wire in wires_driven_by_so_far) and (
                                    wires_driven_by_so_far[driven_wire]
                                    is not None):
                            ...
                            if is_zero_clk_has_delay:
                                delay_offset_when_driven[
                                    driven_wire] = delay_offset_when_driven[
                                        driving_wire]
                            #~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
                            ...
                            if driven_wire in logic.state_regs and logic.state_regs[
                                    driven_wire].is_volatile:
                                continue
        ...
        '''
                ...
                if not already_fully_driven and not incorrect_stage_for_submodule:
                    ...
                    if C_TO_LOGIC.LOGIC_NEEDS_CLOCK_ENABLE(
                        ...
                        submodule_input_port_driving_wires.append(
                            ce_driving_wire)
                    ...
                    if submodule_has_all_inputs_driven:
                        for input_port_name in submodule_logic.inputs:
                            driving_wire = C_TO_LOGIC.GET_SUBMODULE_INPUT_PORT_DRIVING_WIRE(
                                logic, submodule_inst, input_port_name)
                            submodule_input_port_driving_wires.append(
                                driving_wire)
                            input_port_const_driving_wire = C_TO_LOGIC.FIND_CONST_DRIVING_WIRE(
                                driving_wire, logic)
                            if driving_wire not in wires_driven_by_so_far and input_port_const_driving_wire is None:
                                submodule_has_all_inputs_driven = False
                                if bad_inf_loop:
                                    print("!! " + submodule_inst +
                                          " input wire " + input_port_name +
                                          " not driven yet")
                    ...
                    if submodule_has_all_inputs_driven:
                        fully_driven_submodule_inst_2_logic[
                            submodule_inst] = submodule_logic
                        ...
                        if is_zero_clk_has_delay:
                            ...
                            for submodule_input_port_driving_wire in submodule_input_port_driving_wires:
                                ...
                                if C_TO_LOGIC.FIND_CONST_DRIVING_WIRE(
                                    ...
                                    input_port_delay_offsets.append(
                                        delay_offset)
                            max_input_port_delay_offset = max(
                            ...
                            rv.zero_clk_submodule_start_offset[
                                submodule_inst] = max_input_port_delay_offset
                            ...
                            if submodule_delay > 0:
                                abs_delay_end_offset = submodule_delay + rv.zero_clk_submodule_start_offset[
                                    submodule_inst] - 1
                            else:
                                abs_delay_end_offset = rv.zero_clk_submodule_start_offset[
                                    submodule_inst]
                            rv.zero_clk_submodule_end_offset[
                                submodule_inst] = abs_delay_end_offset
                            ...
                            if submodule_delay > 0:
                                ...
                                end_offset = rv.zero_clk_submodule_end_offset[
                                    submodule_inst]
                                for abs_delay in range(start_offset,
                                                       end_offset + 1):
                                    if abs_delay < 0:
                                        ...
                                        sys.exit(-1)
                                    # Submodule isnts
                                    if not (abs_delay in rv.
                                            zero_clk_per_delay_submodules_map):
                                        rv.zero_clk_per_delay_submodules_map[
                                            abs_delay] = []
                                    if not (submodule_inst in rv.
                                            zero_clk_per_delay_submodules_map[
                                                abs_delay]):
                                        rv.zero_clk_per_delay_submodules_map[
                                            abs_delay].append(submodule_inst)
            ...
            if bad_inf_loop:
                print("fully_driven_submodule_inst_this_level_2_logic",
                      fully_driven_submodule_inst_this_level_2_logic)
            #/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\

            # Im dumb
            submodule_level_iteration_has_submodules = len(
                fully_driven_submodule_inst_this_level_2_logic) > 0
            ...
            for submodule_inst in fully_driven_submodule_inst_this_level_2_logic:
                ...
                for output_port in submodule_logic.outputs:
                    ...
                    wire_to_remaining_clks_before_driven[
                        submodule_output_wire] = submodule_latency_from_container_logic
                    ...
                    if is_zero_clk_has_delay:
                        ...
                        abs_delay_offset = rv.zero_clk_submodule_end_offset[
                            submodule_inst]
                        delay_offset_when_driven[
                            submodule_output_wire] = abs_delay_offset
                        # ONly +1 if delay > 0
                        if submodule_delay > 0:
                            delay_offset_when_driven[
                                submodule_output_wire] += 1  # "+1" Was seeing stacked parallel submodules where they dont exist
                    # ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
            ####################################################################################################################
            ...
            submodule_level_prepend_text = "  " + " " + "-- SUBMODULE LEVEL " + str(
                submodule_level) + "\n"
      ...
      '''
            ...
            for wire in wire_to_remaining_clks_before_driven:
                if wire_to_remaining_clks_before_driven[wire] == 0:
                    if wire not in wires_driven_by_so_far:
                        if bad_inf_loop:
                            print("wire remaining clks done, is driven now",
                                  wire)
        ...
        if is_zero_clk_has_delay:
            ...
            if len(list(rv.zero_clk_per_delay_submodules_map.keys())) == 0:
                ...
                rv.zero_clk_max_delay = max(
                    rv.zero_clk_per_delay_submodules_map.keys(
                    )) + 1  # +1 since 0 indexed
        # ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
        ...
        for wire in wire_to_remaining_clks_before_driven:
            #if wire_to_remaining_clks_before_driven[wire] >= 0:
            wire_to_remaining_clks_before_driven[
                wire] = wire_to_remaining_clks_before_driven[wire] - 1
        ...
        if num_volatiles == 0:
            if est_total_latency is not None:
                ...
                if PIPELINE_DONE() and (my_total_latency != est_total_latency):
                    print(
                        "Seems like pipeline is done before or after last stage?"
                    )
                    print(
                        "inst_name", inst_name,
                        timing_params.GET_HASH_EXT(TimingParamsLookupTable,
                                                   parser_state))
                    print("est_total_latency", est_total_latency,
                          "calculated total_latency", my_total_latency)
                    print("timing_params._slices", timing_params._slices)
...
def SLICE_DOWN_HIERARCHY_WRITE_VHDL_PACKAGES(inst_name,
                                             logic,
                                             new_slice_pos,
                                             parser_state,
                                             TimingParamsLookupTable,
  ...
  '''
    ...
    if print_debug:
        print("SLICE_DOWN_HIERARCHY_WRITE_VHDL_PACKAGES", inst_name,
              new_slice_pos, "write_files", write_files)
    ...
    if len(logic.submodule_instances) > 0:
        ...
        # Clamp to max?
        max_delay = max(
            zero_clk_pipeline_map.zero_clk_per_delay_submodules_map.keys())
        ...
        if delay_offset - 1 in zero_clk_pipeline_map.zero_clk_per_delay_submodules_map:
            prev_submodule_insts = zero_clk_pipeline_map.zero_clk_per_delay_submodules_map[
                delay_offset - 1]
        next_submodule_insts = []
        if delay_offset + 1 in zero_clk_pipeline_map.zero_clk_per_delay_submodules_map:
            next_submodule_insts = zero_clk_pipeline_map.zero_clk_per_delay_submodules_map[
                delay_offset + 1]
        ...
        for submodule_inst in submodule_insts:
            ...
            if not submodule_timing_params.params_are_fixed:
                ...
                TimingParamsLookupTable = SLICE_DOWN_HIERARCHY_WRITE_VHDL_PACKAGES(
                    submodule_inst_name, submodule_logic, slice_pos,
                    parser_state, TimingParamsLookupTable, skip_boundary_slice,
                    write_files)
    ...
    if write_files:
        ...
        VHDL.WRITE_LOGIC_ENTITY(inst_name, logic, syn_out_dir, parser_state,
                                TimingParamsLookupTable)
...
def RECURSIVE_SET_NON_FIXED_TO_ZERO_CLK_TIMING_PARAMS(inst_name,
                                                      parser_state,
                                                      TimingParamsLookupTable,
    ...
    if not timing_params.params_are_fixed or override_fixed:
        ...
        for local_sub_inst in logic.submodule_instances:
            ...
            TimingParamsLookupTable = RECURSIVE_SET_NON_FIXED_TO_ZERO_CLK_TIMING_PARAMS(
                sub_inst, parser_state, TimingParamsLookupTable,
                override_fixed)
...
def ADD_SLICES_DOWN_HIERARCHY_TIMING_PARAMS_AND_WRITE_VHDL_PACKAGES(
    ...
    for current_slice_i in current_slices:
        ...
        TimingParamsLookupTable = SLICE_DOWN_HIERARCHY_WRITE_VHDL_PACKAGES(
            inst_name, logic, current_slice_i, parser_state,
            TimingParamsLookupTable, skip_boundary_slice, write_files_in_loop,
            rounding_so_fuck_it)
        # Might be bad slice
        if type(TimingParamsLookupTable) is int:
            return TimingParamsLookupTable
    ...
    if write_files:
        ...
        for inst_name_to_wr in TimingParamsLookupTable:
            ...
            if wr_timing_params.GET_TOTAL_LATENCY(parser_state,
                                                  TimingParamsLookupTable) > 0:
                wr_syn_out_dir = GET_OUTPUT_DIRECTORY(wr_logic)
                if not os.path.exists(wr_syn_out_dir):
                    os.makedirs(wr_syn_out_dir)
                VHDL.WRITE_LOGIC_ENTITY(inst_name_to_wr, wr_logic,
                                        wr_syn_out_dir, parser_state,
                                        TimingParamsLookupTable)

    return TimingParamsLookupTable


def GET_CLK_TO_MHZ_AND_CONSTRAINTS_PATH(parser_state,
                                        inst_name=None,
                                        allow_no_syn_tool=False):
    ext = None
    ...
    if SYN_TOOL is OPEN_TOOLS:
        ...
        for clock_name in clock_name_to_mhz:
            ...
            if clock_mhz is None:
                ...
                clock_mhz = INF_MHZ
            ns = (1000.0 / clock_mhz)
            f.write("create_clock -add -name " + clock_name + " -period " +
                    str(ns) + " -waveform {0 " + str(ns / 2.0) +
                    "} [get_nets " + clock_name + "]\n")
...
def TOOL_DOES_PNR():
    ...
    # Uses PNR
    elif (SYN_TOOL is QUARTUS) or (SYN_TOOL is OPEN_TOOLS) or (SYN_TOOL is
                                                               EFINITY):
        return True
...
def WRITE_FINAL_FILES(multimain_timing_params, parser_state):
        ...
        if func_name in parser_state.FuncToInstances:
            ...
            for inst_name in parser_state.FuncToInstances[func_name]:
                ...
                VHDL.WRITE_LOGIC_ENTITY(
                    inst_name, blackbox_func_logic, bb_out_dir, parser_state,
    ...
    if SYN_TOOL is VIVADO:
        ...
        for line in tcl.split('\n'):
            ...
            if line.startswith("read_vhdl") or line.startswith(
                    "add_files") or line.startswith("set_property"):
        ...
        tcl = QUARTUS.GET_SH_TCL(top_entity_name, vhdl_files_texts,
                                 constraints_filepath, parser_state)
        rv_lines = []
        for line in tcl.split('\n'):
...
def GET_MOST_RECENT_OR_DEFAULT_SWEEP_STATE(parser_state,
                                           multimain_timing_params):
    # Try to use cache
        ...
        for main_inst_name in parser_state.main_mhz:
            ...
            if target_mhz is not None:
                ...
                sweep_state.inst_sweep_state[
                    main_inst_name].hier_sweep_mult = 0.0
                if delay > 0.0 and func_logic.CAN_BE_SLICED():
                    sweep_state.inst_sweep_state[
                        main_inst_name].slice_ep = SLICE_EPSILON(delay)
                    ...
                    sweep_state.inst_sweep_state[
                        main_inst_name].hier_sweep_mult = hier_sweep_mult
...
def REDUCE_SLICE_STEP(slice_step, total_latency, epsilon):
    n = 0
    while True:
        maybe_new_slice_step = 1.0 / ((SLICE_STEPS_BETWEEN_REGS + 1 + n) *
                                      (total_latency + 1))
...
# Wow this is hack AF
def GET_MAIN_INSTS_FROM_PATH_REPORT(path_report, parser_state,
                                    multimain_timing_params):
    ...
    main_insts = set()
    all_main_insts = list(
        reversed(sorted(list(parser_state.main_mhz.keys()), key=len)))
    ...
    for main_inst in all_main_insts:
        ...
        main_vhdl_entity_name = VHDL.GET_ENTITY_NAME(
            main_inst, main_logic,
            multimain_timing_params.TimingParamsLookupTable, parser_state)
        # OPEN_TOOLs reports in lower case
        if path_report.start_reg_name.lower().startswith(
                main_vhdl_entity_name.lower()):
            start_reg_main = main_inst
        if path_report.end_reg_name.lower().startswith(
                main_vhdl_entity_name.lower()):
    ...
    if len(main_insts) == 0:
        ...
        for netlist_resource in all_netlist_resources:
            ...
            for main_inst in all_main_insts:
                ...
                # OPEN_TOOLs reports in lower case
                if netlist_resource.lower().startswith(
                        main_vhdl_entity_name.lower()):
...
def GET_REGISTERS_ESTIMATE_TEXT_AND_FFS(logic, inst_name, parser_state,
                                        TimingParamsLookupTable, ff_est_cache):
    timing_params = TimingParamsLookupTable[inst_name]
    hash_ext = timing_params.GET_HASH_EXT(TimingParamsLookupTable,
                                          parser_state)
    ...
    if latency > 0:
        if VHDL.LOGIC_IS_RAW_HDL(logic):
            ...
            pipeline_map = GET_PIPELINE_MAP(inst_name, logic, parser_state,
                                            TimingParamsLookupTable)
            ...
            pipeline_text = ""
            for stage in range(
                    0, latency):  # Last stage never has regs, dont print
                stage_ffs = 0
                stage_wires_text = ""
                for wire in pipeline_hdl_params.wires_to_decl:
                    wire_start_stage, wire_end_stage = pipeline_hdl_params.wire_to_reg_stage_start_end[
                        wire]
    ...
    for sub_inst in logic.submodule_instances:
        ...
        sub_inst_text, sub_inst_ffs = GET_REGISTERS_ESTIMATE_TEXT_AND_FFS(
            sub_logic, sub_full_inst, parser_state, TimingParamsLookupTable,
            ff_est_cache)
...
def WRITE_REGISTERS_ESTIMATE_FILE(
        parser_state,
        multimain_timing_params_or_TimingParamsLookupTable,
        inst_name=None):
    ...
    for main_func in parser_state.main_mhz:
        ...
        main_func_text, main_func_ffs = GET_REGISTERS_ESTIMATE_TEXT_AND_FFS(
            main_logic, main_func, parser_state, TimingParamsLookupTable,
            ff_est_cache)
        text += main_func_text
        text += "\n"
...
def DO_THROUGHPUT_SWEEP(parser_state,
                        coarse_only=False,
    ...
    ZeroClockTimingParamsLookupTable = GET_ZERO_CLK_TIMING_PARAMS_LOOKUP(
        parser_state)
    ...
    # If comb. logic only
    if comb_only:
        print("Running synthesis on comb. logic only top level ...",
              flush=True)
    ...
    if coarse_only:
        ...
        if len(parser_state.main_mhz) > 1:
            ...
            if len(possible_mains) == 1:
                ...
                raise Exception(
                    f"No main functions are elligible for pipelining.")
            else:
                raise Exception(
        ...
        target_mhz = GET_TARGET_MHZ(main_func, parser_state)
        if target_mhz is None:
            print("Main function:", main_func,
                  "does not have a set target frequency.")
            target_mhz = INF_MHZ
        ...
        inst_sweep_state, working_slices, multimain_timing_params.TimingParamsLookupTable = DO_COARSE_THROUGHPUT_SWEEP(
            main_func, target_mhz, inst_sweep_state, parser_state,
            starting_guess_latency, do_incremental_guesses,
            max_allowed_latency_mult, stop_at_n_worse_result, stop_at_latency)
...
def DO_MIDDLE_OUT_THROUGHPUT_SWEEP(parser_state, sweep_state):
    ...
    )  # hacky indicator of if printed slicing of func yet
    ...
    def hier_sweep_mult_func(target_path_delay_ns, func_path_delay_ns):
        return (target_path_delay_ns /
                func_path_delay_ns) + HIER_SWEEP_MULT_INC
    ...
    while not sweep_state.met_timing:
        ...
        while keep_try_for_timing_params:
            ...
            sweep_state.multimain_timing_params.TimingParamsLookupTable = GET_ZERO_CLK_TIMING_PARAMS_LOOKUP(
                parser_state)
            for main_inst in parser_state.main_mhz:
                sweep_state.inst_sweep_state[
                    main_inst].smallest_not_sliced_hier_mult = INF_HIER_MULT
            ...
            while collecting_modules_to_pipeline:
                ...
                for func_inst in list(lowest_level_insts_to_pipeline
                    ...
                    func_path_delay_ns = float(
                        func_logic.delay) / DELAY_UNIT_MULT
                    ...
                    target_path_delay_ns = 1000.0 / target_mhz
                    coarse_target_mhz = target_mhz * sweep_state.inst_sweep_state[
                        main_func].coarse_sweep_mult

                    if (sweep_state.inst_sweep_state[main_func].hier_sweep_mult
                        ...
                        if func_path_delay_ns > 0.0:
                            hsm_i = hier_sweep_mult_func(
                                target_path_delay_ns, func_path_delay_ns)
                            if hsm_i < sweep_state.inst_sweep_state[
                                    main_func].smallest_not_sliced_hier_mult:
                                sweep_state.inst_sweep_state[
                                    main_func].smallest_not_sliced_hier_mult = hsm_i
                        ...
                        func_inst_timing_params = sweep_state.multimain_timing_params.TimingParamsLookupTable[
                            func_inst]
                        ...
                        for local_sub_inst in func_logic.submodule_instances:
                            ...
                            sub_logic = parser_state.LogicInstLookupTable[
                                sub_inst_name]
                            if sub_logic.delay is not None:
                                sub_func_path_delay_ns = float(
                                    sub_logic.delay) / DELAY_UNIT_MULT
                                if (sweep_state.inst_sweep_state[main_func].
                                        hier_sweep_mult *
                                    ...
                                    lowest_level_insts_to_pipeline.discard(
                                        func_inst)
                                    collecting_modules_to_pipeline = True
                                else:
            ...
            for func_inst in lowest_level_insts_to_pipeline:
                ...
                if containr_inst in lowest_level_insts_to_pipeline:
                    print(containr_inst, "and submodule", func_inst,
            ...
            for func_inst in sorted(list(lowest_level_insts_to_pipeline)
                ...
                if (
                        func_path_delay_ns * sweep_state.
                        inst_sweep_state[main_func].coarse_sweep_mult
                    ...
                    if cache_key not in printed_slices_cache:
                        print("Slicing w/ IO regs:",
                              func_logic.func_name,
                              ", mult =",
                              sweep_state.inst_sweep_state[main_func].
                              coarse_sweep_mult,
                              slices,
                              flush=True)
                ...
                # Cached result?
                    ...
                    needs_out_regs = True
                    if cache_key not in printed_slices_cache:
                        print("Cached coarse grain slicing:",
                              func_logic.func_name,
                              ", target MHz =",
                              coarse_target_mhz,
                              slices,
                ...
                # Manual coarse pipelining
                    ...
                    if func_inst not in sweep_state.inst_sweep_state:
                        sweep_state.inst_sweep_state[
                            func_inst] = InstSweepState()
                    func_inst_timing_params = sweep_state.multimain_timing_params.TimingParamsLookupTable[
                        func_inst]
                    if cache_key not in printed_slices_cache:
                        print("Coarse grain sweep slicing",
                              func_logic.func_name,
                              ", target MHz =",
                              coarse_target_mhz,
                              flush=True)
                    ...
                    coarse_sweep_initial_clks = int(
                        math.ceil(
                            (func_path_delay_ns * sweep_state.
                             inst_sweep_state[main_func].coarse_sweep_mult) /
                            target_path_delay_ns)) - 1
                    allowed_worse_results = int(MAX_N_WORSE_RESULTS /
                                                coarse_sweep_initial_clks)
                    if allowed_worse_results == 0:
                        allowed_worse_results = 1
                    print("Allowed worse results in coarse sweep:",
                          allowed_worse_results)
                    # Why not do middle out again? All the way down? Because complicated?weird do later
                    sweep_state.inst_sweep_state[
                        func_inst], working_slices, inst_TimingParamsLookupTable = DO_COARSE_THROUGHPUT_SWEEP(
                            func_inst,
                            coarse_target_mhz,
                    ...
                    if not sweep_state.inst_sweep_state[func_inst].met_timing:
                        ...
                        if sweep_state.inst_sweep_state[
                                main_func].smallest_not_sliced_hier_mult != INF_HIER_MULT:
                            keep_try_for_timing_params = True
                            ...
                            if sweep_state.inst_sweep_state[
                                    main_func].smallest_not_sliced_hier_mult == sweep_state.inst_sweep_state[
                                        main_func].hier_sweep_mult:
                                sweep_state.inst_sweep_state[
                                    main_func].hier_sweep_mult += HIER_SWEEP_MULT_INC
                                print(
                                    main_func,
                                ...
                                # Normal case
                                sweep_state.inst_sweep_state[
                                    main_func].hier_sweep_mult = sweep_state.inst_sweep_state[
                                        main_func].smallest_not_sliced_hier_mult
                                print(
                                    main_func, "hierarchy sweep multiplier:",
                                    sweep_state.inst_sweep_state[main_func].
                                    hier_sweep_mult)
                            sweep_state.inst_sweep_state[
                                main_func].best_guess_sweep_mult = 1.0
                            sweep_state.inst_sweep_state[
                                main_inst].coarse_sweep_mult = 1.0
                        else:
                    ...
                    needs_out_regs = True
                    if cache_key not in printed_slices_cache:
                        print("Coarse gain confirmed slicing (w/ IO regs):",
                              func_logic.func_name,
                              ", target MHz =",
                              coarse_target_mhz,
                              slices,
                ...
                sweep_state.multimain_timing_params.TimingParamsLookupTable = (
                    ADD_SLICES_DOWN_HIERARCHY_TIMING_PARAMS_AND_WRITE_VHDL_PACKAGES(
                        ...
                        sweep_state.multimain_timing_params.
                        TimingParamsLookupTable, write_files))
                ...
                sweep_state.multimain_timing_params.TimingParamsLookupTable[
                    func_inst] = func_inst_timing_params
            ...
            for main_inst in parser_state.main_mhz:
                main_inst_timing_params = sweep_state.multimain_timing_params.TimingParamsLookupTable[
                    main_inst]
                if main_inst_timing_params.params_are_fixed:
                    continue
                main_func_logic = parser_state.LogicInstLookupTable[main_inst]
                main_func_path_delay_ns = float(
                    main_func_logic.delay) / DELAY_UNIT_MULT
                target_mhz = GET_TARGET_MHZ(main_func, parser_state)
                target_path_delay_ns = 1000.0 / target_mhz
                clks = int(
                    math.ceil(
                        (main_func_path_delay_ns * sweep_state.
                         inst_sweep_state[main_inst].best_guess_sweep_mult) /
                        target_path_delay_ns)) - 1
                if clks <= 0:
                    continue
                slices = GET_BEST_GUESS_IDEAL_SLICES(clks)
                print("Best guess slicing:",
                ...
                sweep_state.multimain_timing_params.TimingParamsLookupTable[
                    main_inst] = main_inst_timing_params
        ...
        for inst_name_to_wr in sweep_state.multimain_timing_params.TimingParamsLookupTable:
            wr_logic = parser_state.LogicInstLookupTable[inst_name_to_wr]
            wr_timing_params = sweep_state.multimain_timing_params.TimingParamsLookupTable[
                inst_name_to_wr]
            if wr_timing_params.GET_TOTAL_LATENCY(
                    parser_state, sweep_state.multimain_timing_params.
                    TimingParamsLookupTable) > 0:
                entity_name = VHDL.GET_ENTITY_NAME(
                    inst_name_to_wr, wr_logic, sweep_state.
                    multimain_timing_params.TimingParamsLookupTable,
                    parser_state)
                if entity_name not in entities_written:
                    ...
                    if not os.path.exists(wr_filename):
                        VHDL.WRITE_LOGIC_ENTITY(
                            inst_name_to_wr, wr_logic, wr_syn_out_dir,
                            parser_state, sweep_state.multimain_timing_params.
                            TimingParamsLookupTable)
        # Write estimate of FF usage
        WRITE_REGISTERS_ESTIMATE_FILE(parser_state,
                                      sweep_state.multimain_timing_params)
        ...
        for main_func in parser_state.main_mhz:
            ...
            main_func_timing_params = sweep_state.multimain_timing_params.TimingParamsLookupTable[
                main_func]
            print(main_func,
                  ":",
        ...
        for reported_clock_group in sweep_state.timing_report.path_reports:
            path_report = sweep_state.timing_report.path_reports[
            ...
            if len(parser_state.main_mhz
                   ) == 1:  # hacky work around for pyrtl really...
                ...
                main_insts = GET_MAIN_INSTS_FROM_PATH_REPORT(
                    ...
                    sweep_state.multimain_timing_params)
            if len(main_insts) <= 0:
                print("No main functions in timing report for path group:",
                      reported_clock_group)
                #print(path_report.start_reg_name)
                #print(path_report.end_reg_name)
                #print(path_report.netlist_resources)
                for main_inst in parser_state.main_mhz:
                    main_logic = parser_state.LogicInstLookupTable[main_inst]
                    main_vhdl_entity_name = VHDL.GET_ENTITY_NAME(
                        main_inst, main_logic, sweep_state.
                        multimain_timing_params.TimingParamsLookupTable,
                        parser_state)
                    print(main_vhdl_entity_name)
                sys.exit(-1)
            ...
            for main_inst in main_insts:
                ...
                # Print and log
                print(
                    "{} Clock Goal: {:.2f} (MHz) Current: {:.2f} (MHz)({:.2f} ns) {} clks"
                    .format(main_func_logic.func_name, target_mhz, curr_mhz,
                            path_report.path_delay_ns, latency),
                    flush=True)
                best_mhz_so_far = 0.0
                if len(sweep_state.inst_sweep_state[main_inst].mhz_to_latency
                       ) > 0:
                    best_mhz_so_far = max(
                        sweep_state.inst_sweep_state[main_inst].mhz_to_latency.
                        keys())
                best_mhz_this_latency = 0.0
                if latency in sweep_state.inst_sweep_state[
                        main_inst].latency_to_mhz:
                    best_mhz_this_latency = sweep_state.inst_sweep_state[
                        main_inst].latency_to_mhz[latency]
                ...
                if not main_met_timing:
                    ...
                    if main_func_logic.CAN_BE_SLICED():
                        ...
                        new_best_guess_sweep_mult = sweep_state.inst_sweep_state[
                            main_inst].best_guess_sweep_mult * latency_mult
                        ...
                        if new_best_guess_sweep_mult > BEST_GUESS_MUL_MAX:  #15 like? main_max_allowed_latency_mult  2.0 magic?
                            ...
                            if (sweep_state.inst_sweep_state[main_inst].
                                    coarse_sweep_mult + COARSE_SWEEP_MULT_INC
                                ) <= COARSE_SWEEP_MULT_MAX:  #1.5: # MAGIC?
                                sweep_state.inst_sweep_state[
                                    main_inst].best_guess_sweep_mult = 1.0
                                sweep_state.inst_sweep_state[
                                    main_inst].coarse_sweep_mult += COARSE_SWEEP_MULT_INC
                                print(
                                    "Coarse synthesis sweep multiplier:",
                                    sweep_state.inst_sweep_state[main_inst].
                                    coarse_sweep_mult)
                                made_adj = True
                            elif sweep_state.inst_sweep_state[
                                    main_inst].smallest_not_sliced_hier_mult != INF_HIER_MULT:
                                print(
                                    "Trying to pipeline smaller modules instead..."
                                )
                                ...
                                if sweep_state.inst_sweep_state[
                                    ...
                                    sweep_state.inst_sweep_state[
                                        main_inst].hier_sweep_mult = sweep_state.inst_sweep_state[
                                            main_inst].smallest_not_sliced_hier_mult
                                    print(
                            ...
                            sweep_state.inst_sweep_state[
                                main_inst].best_guess_sweep_mult = new_best_guess_sweep_mult
                            print(
                    ...
                    if print_path:
                        ...
                        print(" ~", path_report.path_delay_ns,
                              "ns of logic+routing ~")
...
def DO_COARSE_THROUGHPUT_SWEEP(inst_name,
                               target_mhz,
                               inst_sweep_state,
                               parser_state,
                               starting_guess_latency=None,
    ...
    while True:
        ...
        TimingParamsLookupTable = GET_ZERO_CLK_TIMING_PARAMS_LOOKUP(
            parser_state)
        ...
        if len(TimingParamsLookupTable[inst_name]._slices) != 0:
            print("Not starting with comb logic for main? sliced already?",
                  TimingParamsLookupTable[inst_name]._slices)
            sys.exit(-1)

        # Apply slices to main funcs
        TimingParamsLookupTable = ADD_SLICES_DOWN_HIERARCHY_TIMING_PARAMS_AND_WRITE_VHDL_PACKAGES(
            inst_name, logic, best_guess_slices, parser_state,
            TimingParamsLookupTable, write_files)
        ...
        if TimingParamsLookupTable[inst_name]._slices != best_guess_slices:
            print("Tried to slice as ", best_guess_slices, "but got",
                  TimingParamsLookupTable[inst_name]._slices)
            sys.exit(-1)
        ...
        for inst_name_to_wr in TimingParamsLookupTable:
            ...
            if wr_timing_params.GET_TOTAL_LATENCY(parser_state,
                                                  TimingParamsLookupTable) > 0:
                entity_name = VHDL.GET_ENTITY_NAME(inst_name_to_wr, wr_logic,
                                                   TimingParamsLookupTable,
                                                   parser_state)
                if entity_name not in entities_written:
                    entities_written.add(entity_name)
                    ...
                    if not os.path.exists(wr_filename):
                        VHDL.WRITE_LOGIC_ENTITY(inst_name_to_wr, wr_logic,
                                                ...
                                                TimingParamsLookupTable)
        # Write estimate of FF usage
        WRITE_REGISTERS_ESTIMATE_FILE(parser_state, TimingParamsLookupTable,
                                      inst_name)
        ...
        for reported_clock_group in inst_sweep_state.timing_report.path_reports:
            ...
            print(
                "{} Clock Goal: {:.2f} (MHz) Current: {:.2f} (MHz)({:.2f} ns) {} clks"
                .format(func_logic.func_name, target_mhz, curr_mhz,
                        path_report.path_delay_ns, latency),
                flush=True)
            best_mhz_so_far = 0.0
            ...
            if better_mhz:
                ...
                if stop_at_n_worse_result is not None:
                    if inst_sweep_state.worse_or_same_tries_count >= stop_at_n_worse_result:
                        print(logic.func_name, "giving up after",
            ...
            '''
                        ...
                        if inst_sweep_state.last_latency_increase is not None and clk_inc >= inst_sweep_state.last_latency_increase:
                            ...
                            print("Clipped for decreasing jump size to", clks,
                                  "clocks...")
...
# Return slices
def EXPAND_STAGES_VIA_ADJ_COUNT(missing_stages, current_slices, slice_step,
                                state, min_dist):
    print("<<<<<<<<<<< EXPANDING", missing_stages, "VIA_ADJ_COUNT:",
          current_slices)
    ...
    for missing_stage in missing_stages:
        slice_per_stage[
            missing_stage] = slice_per_stage[missing_stage] + expansion
    ...
    if len(stages_to_shrink) > 0:
        ...
        for stage in stages_to_shrink:
            print("Shrinking stage", stage, "curr size:",
                  slice_per_stage[stage])
    ...
    for clk_mhz in mhz_to_latency:
        ...
        for div_i in range(2, max_div + 1):
            ...
            if div_clk_mhz in mhz_to_latency:
                ...
                text += str(clk_mhz) + "  " + str(
                    high_clk_latency_ns
                ) + "  " + str(div_clk_mhz) + "  " + str(
                    div_clock_latency_ns) + " " + str(n_parallel) + " " + str(
                        deser_delay) + "  " + str(ser_delay) + "  " + str(
                            total_latency) + "\n"
...
def LOGIC_IS_ZERO_DELAY(logic, parser_state):
    ...
    # Black boxes have no known delay to the tool
        ...
        return True
            ...
            C_TO_LOGIC.BIN_OP_SL_NAME) or logic.func_name.startswith(
...
def IS_USER_CODE(logic, parser_state):
    ...
    user_code = (not logic.is_c_built_in
                 and not SW_LIB.IS_AUTO_GENERATED(logic))
...
def GET_PATH_DELAY_CACHE_DIR(logic, parser_state):
    PATH_DELAY_CACHE_DIR = C_TO_LOGIC.EXE_ABS_DIR(
    ) + "/../path_delay_cache/" + str(SYN_TOOL.__name__).lower()
...
def GET_CACHED_PATH_DELAY_FILE_PATH(logic, parser_state):
    ...
    file_path = GET_PATH_DELAY_CACHE_DIR(logic,
                                         parser_state) + "/" + key + ".delay"
    ...
    while not all_funcs_done:
        ...
        while still_finding_parallel_funcs:
            ...
            for logic_func_name in parser_state.FuncLogicLookupTable:
                ...
                if logic_func_name in parser_state.FuncToInstances:
                    inst_name = list(
                        parser_state.FuncToInstances[logic_func_name])[0]
                if inst_name is None:
                    #print("Warning?: No logic instance for function:", logic.func_name, "never used?")
                    func_names_done_so_far.append(
                        logic_func_name)  # Helps with % printout?
                    continue
                # All dependencies met?
                all_dep_met = True
                for submodule_inst in logic.submodule_instances:
                    submodule_func_name = logic.submodule_instances[
                        submodule_inst]
                ...
                if all_dep_met:
                    ...
                    cached_path_delay = GET_CACHED_PATH_DELAY(
                        logic, parser_state)
                    if cached_path_delay is not None:
                        ...
                        if cached_path_delay > 0.0 and logic.delay == 0:
                            print("Have timing path of", cached_path_delay,
                                  "ns")
                            print(
        ...
        for logic_func_name in parallel_func_names:
            ...
            if logic.func_name in parser_state.FuncToInstances:
                inst_name = list(
                    parser_state.FuncToInstances[logic.func_name])[0]
            if inst_name is None:
                #print "Warning?: No logic instance for function:", logic.func_name, "never used?"
                continue
            ...
            ) > 0 and not logic.is_vhdl_text_module:  # TODO make pipeline map return empty instead of check?
                zero_clk_pipeline_map = GET_ZERO_CLK_PIPELINE_MAP(
                    inst_name, logic, parser_state
        ...
        # Parallelized
        NUM_PROCESSES = int(
            open(C_TO_LOGIC.EXE_ABS_DIR() + "/../num_processes.cfg",
                 'r').readline())
        ...
        for logic_func_name in parallel_func_names:
            ...
            if not IS_USER_CODE(logic, parser_state):
                ...
                PATH_DELAY_CACHE_DIR = GET_PATH_DELAY_CACHE_DIR(
                    logic, parser_state)
...
def WRITE_ALL_ZERO_CLK_VHDL(parser_state, ZeroClkTimingParamsLookupTable):
    ...
    for func_name in parser_state.FuncLogicLookupTable:
        if func_name in parser_state.FuncToInstances:
            ...
            # ONly write non vhdl
            if logic.is_vhdl_func or logic.is_vhdl_expr or (
                    logic.func_name == C_TO_LOGIC.VHDL_FUNC_NAME):
                continue
            ...
            if not os.path.exists(syn_out_dir):
                os.makedirs(syn_out_dir)
            VHDL.WRITE_LOGIC_ENTITY(inst_name, logic, syn_out_dir,
                                    parser_state,
...
def GET_VHDL_FILES_TCL_TEXT_AND_TOP(multimain_timing_params,
                                    parser_state,
                                    inst_name=None,
                                    is_final_top=False):
    ...
    needs_global_t = VHDL.NEEDS_GLOBAL_WIRES_VHDL_PACKAGE(
        parser_state) and not inst_name  # is multimain
    if inst_name:
    ...
    # Top not shared
    top_entity_name = VHDL.GET_TOP_ENTITY_NAME(parser_state,
                                               multimain_timing_params,
                                               inst_name, is_final_top)
    if inst_name:
        Logic = parser_state.LogicInstLookupTable[inst_name]
    ...
    while len(inst_names) > 0:
        ...
        for inst_name_i in inst_names:
            ...
            if logic_i.is_clock_crossing:
                continue
            entity_filename = VHDL.GET_ENTITY_NAME(
                inst_name_i, logic_i,
                multimain_timing_params.TimingParamsLookupTable,
                parser_state) + ".vhd"

...
def WRITE_POST_PREPROCESS_GEN_CODE(preprocessed_c_text, regenerate_files):  
  new_regenerate_files = set()
...
def GEN_EMPTY_GENERATED_HEADERS(all_code_files, inital_missing_files):
...
'''
    ...
    if not os.path.exists(path) or "#include" not in open(path, "r").read():
      open(path,'w').write(text)

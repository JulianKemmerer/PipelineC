...
Usage (mirrors pypelinec's own args -- pass whatever you'd normally pass to
`pypelinec ... --sim ...`, this script runs it once as given and once again
...
_PYPELINEC = os.path.join(_HERE, "pypelinec")
...
def _run_pypelinec(extra_args, label):
    cmd = [sys.executable, _PYPELINEC] + extra_args
...
def _find_c_file(pypelinec_args):
    for a in pypelinec_args:
        if not a.startswith("-"):
            return a
    return "design"
...
def main():
    ...
    args, pypelinec_args = parser.parse_known_args()

    if "--cocotb" in pypelinec_args or "--ghdl" in pypelinec_args:
        print(
    ...
    if args.out_dir is not None:
        ...
        c_file_base = os.path.basename(_find_c_file(pypelinec_args))
    ...
    if needs_shared_build:
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=2) as pool:
            ...
            vhdl_future = pool.submit(_run_pypelinec, vhdl_args, "VHDL/cocotb")
            native_stdout, native_returncode = native_future.result()
            vhdl_stdout, vhdl_returncode = vhdl_future.result()
    ...
    if native_returncode != 0:
        sim_exit_failures.append(f"native sim exited {native_returncode}")
    if vhdl_returncode != 0:
        sim_exit_failures.append(f"VHDL/cocotb sim exited {vhdl_returncode}")

    if first_mismatch_cycle is None:
        if sim_exit_failures:
            print(
                f"MISMATCH: sim_print(debug=True) output matched cycle-for-cycle, "
                f"but {'; '.join(sim_exit_failures)} -- a passing diff does not "
                f"cover whatever happened after the last debug print (e.g. both "
                f"sims failing identically)."
            )
            print(f"Full logs: {native_log_path}  {vhdl_log_path}")
            return 1
        print(

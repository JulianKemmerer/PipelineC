...
def SYN_AND_REPORT_TIMING_NEW(parser_state,  multimain_timing_params, inst_name = None, total_latency = None, hash_ext = None, use_existing_log_file = True):
  ...
  if os.path.exists(log_to_read) and use_existing_log_file:
    ...
    f=open(tcl_path,'w')
    f.write('''
prj_project new -name "''' + top_entity_name + '''" -impl "impl1" -dev ''' + parser_state.part + ''' -synthesis "''' + DIAMOND_TOOL + '''"
    ...
    for path_text in maybe_path_texts:
      if "Source:" in path_text:
        ...
        if path_report.path_group in self.path_reports:
          if path_report.path_delay_ns > self.path_reports[path_report.path_group].path_delay_ns:
            do_add = True
        else:
          do_add = True
        if do_add:

...
Covers:
  1. A generic (any_integer_t) registration resolves for a width no exact
     registration names, and the design's `+` uses it (a soft_add_ripple_*
  ...
  2. Memoization: two call sites at the same width share one Logic() entity
     (one soft_add_ripple_uint9_t_uint9_t func_name, two instances).
...
register_operator("GT", any_integer_t, any_integer_t, make_soft_cmp_sub("GT"))
...
def test_generic_operator_registry():
    ...
    soft_add_names = {n for n in all_instantiated if "soft_add_ripple" in n}
    assert soft_add_names, (
        f"expected a soft_add_ripple_* entity to be instantiated for the "
    ...
    plus_insts = [
        fn for fn in add_twice_logic.submodule_instances.values() if "soft_add_ripple" in fn
    ]
    ...
    assert not any(
        "soft_add_ripple" in n and "uint7_t" in n for n in all_instantiated
    ...
    soft_cmp_names = {n for n in all_instantiated if "soft_cmp_sub" in n}
    assert soft_cmp_names, (
        f"expected a soft_cmp_sub_* entity for the registered GT operator; "
        f"instantiated={sorted(all_instantiated)}"
    )
    assert not any(

...
# AUTOMCP end to end under a real Vivado sweep (driven by automcp_sweep_test.py):
# a ~16-round add/xor mixing chain far slower than one 100 MHz clock period,
# behind make_stream_interface_automcp. The sweep must raise the multi-cycle
...
#   AUTOMCP_SWEEP_START  start_latency= (unset: default start of 1)
#   AUTOMCP_SWEEP_MAX    max_latency=   (unset: no cap)
import os
...
mix_mcp, mix_mcp_t = make_stream_interface_automcp(
    mix,
    start_latency=_env_int("AUTOMCP_SWEEP_START"),
    max_latency=_env_int("AUTOMCP_SWEEP_MAX"),
)


@MAIN(100.0)
@hw_func
def automcp_sweep_main() -> uint32_t:
    x: Reg[uint32_t] = 1
    cycle: Reg[uint16_t]
    ...
    if f.stream_out_if.stream.valid:
        ...
        sim_assert(
            ...
            "AUTOMCP handshake did not wait latency + 1 cycles",
        )

#!/usr/bin/env python3
"""AUTO_FSM resource test: is the FSM actually SMALLER than the combinational
logic it replaces?

Everything else in the AUTO_FSM test suite checks that the feature is correct
...
Method: build auto_fsm_resources_test.py twice.
...
DESIGN = os.path.join(THIS_DIR, "..", "auto_fsm_resources_test.py")
...
def main():
    ...
    base = args.out_dir or os.path.join(THIS_DIR, "auto_fsm_resources_test_out")
    ...
    if not m:
        fail("the full build did not schedule an AUTO_FSM")
    ...
    if ratio < MIN_SHRINK_RATIO:
        fail(
            ...
            f"entire purpose of AUTO_FSM -- if this regressed, the feature is "
            f"buying latency for nothing."
        )

    print(f"AUTO_FSM resource test passed ({ratio:.2f}x smaller).")

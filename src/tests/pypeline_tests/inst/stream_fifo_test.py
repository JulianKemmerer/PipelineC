...
def stream_fifo_test_top(
    in_stream: uint32_stream_t, out_stream: uint32_stream_fb_t
) -> stream_fifo_t:
...
def test_stream_fifo_order_and_backpressure():
    ...
    .out_stream/.in_stream) -- FIFO push/pop/backpressure correctness itself
    ...
    for v in values:
        r = sim_call(
            ...
            uint32_stream_t(data=v, valid=1),
            uint32_stream_fb_t(ready=0),
        )
        assert int(r.in_stream.ready) == 1
    # A push attempt while full must backpressure.
    r = sim_call(
        stream_fifo_test_top,
        uint32_stream_t(data=999, valid=1),
        uint32_stream_fb_t(ready=0),
    )
    assert int(r.in_stream.ready) == 0
    popped = []
    for _ in range(len(values)):
        r = sim_call(
            stream_fifo_test_top,
            uint32_stream_t(data=0, valid=0),

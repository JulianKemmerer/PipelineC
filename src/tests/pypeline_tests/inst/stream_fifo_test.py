...
uint32_plain_t = stream_fifo.stream_intrf.stream_t
CAPACITY = 8


@MAIN
def stream_fifo_test_top(
    in_stream_if: stream_fifo.fwd_t, out_stream_if: stream_fifo.fb_t
) -> stream_fifo_t:
...
def test_stream_fifo_order_and_backpressure():
    ...
    for v in values:
        r = sim_call(
            ...
            stream_fifo.fwd_t(stream=uint32_plain_t(data=v, valid=1)),
            stream_fifo.fb_t(ready=0),
        )
        assert int(r.in_stream_if.ready) == 1
    # A push attempt while full must backpressure.
    r = sim_call(
        stream_fifo_test_top,
        stream_fifo.fwd_t(stream=uint32_plain_t(data=999, valid=1)),
        stream_fifo.fb_t(ready=0),
    )
    ...
    for _ in range(len(values)):
        r = sim_call(
            stream_fifo_test_top,
            stream_fifo.fwd_t(stream=uint32_plain_t(data=0, valid=0)),
            stream_fifo.fb_t(ready=1),
        )

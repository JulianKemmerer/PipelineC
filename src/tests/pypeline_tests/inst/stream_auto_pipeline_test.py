...
@MAIN(50.0)
def stream_auto_pipeline_test_top(
...
def _drive(input_values, out_ready_fn):
    """Feed input_values into stream_auto_pipeline_test_top in order (holding each
...
def test_stream_auto_pipeline_steady_drain():
    sim_reset()
...
def test_stream_auto_pipeline_stall_and_resume():
    sim_reset()
    inputs = [4, 8, 15, 16, 23]
    expected = [int(sim_call(div_inv, x)) for x in inputs]

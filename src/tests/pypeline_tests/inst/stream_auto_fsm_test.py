#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Native-sim handshake test for make_stream_auto_fsm
(include/pypeline/stream/stream_auto_fsm.py). Plain native sim never installs
an AUTO_FSM schedule (pypeline_sim.py run directly), so ADD_FSM.fsm.latency
...
Companion coverage: self_check_stream_auto_fsm_test.py drives the same shape
...
ADD_FSM, ADD_FSM_T = make_stream_auto_fsm(add_ab)
...
def test_ready_deasserts_while_busy():
    ...
    frees combinationally within that cycle (the make_stream_multi_cycle-style
...
if __name__ == "__main__":
    ...
    print("All stream_auto_fsm tests passed.")

#!/usr/bin/env python3
"""AUTO_FSM control-path comparison: is the v3 constant-table control path
actually cheaper than the v2 comparator chains it replaced?

AUTO_FSM shares operations across states, and something has to decode the state
...
Two things are checked, because v3 bought two different wins:
  ...
  area    on auto_fsm_resources_test, where both control paths build, v3 must
...
RESOURCES_DESIGN = os.path.join(THIS_DIR, "..", "auto_fsm_resources_test.py")
DONUT_DESIGN = os.path.join(
    THIS_DIR, "../../../../examples/pypeline/auto_fsm_donut_update.py"
)
...
def schedule_banners(out):
    return sorted(set(re.findall(r"^AUTO_FSM \S+: .*$", out, re.M)))


def check_area(base):
    v2_dir = os.path.join(base, "res_v2")
    v3_dir = os.path.join(base, "res_v3")
    v2_out, _ = run_build(RESOURCES_DESIGN, v2_dir, ["--auto_fsm_ctl", "v2"])
    v3_out, _ = run_build(
        RESOURCES_DESIGN, v3_dir, ["--auto_fsm_ctl", "v3"]
    )
    ...
    print()
    print("=== AUTO_FSM control path (yosys cells, whole design top) ===")
...
def check_timing(base):
    ...
    print()
    print("=== AUTO_FSM control path (vga donut update, at its clock goal) ===")
...
def main():
    ...
    base = args.out_dir or os.path.join(THIS_DIR, "auto_fsm_ctl_compare_test_out")
    check_area(base)
    check_timing(base)
    print("AUTO_FSM control path comparison passed.")

#!/usr/bin/env python3
"""In-process unit tests for the AUTO_FSM scheduler and code generator.
...
DESIGN_SRC = textwrap.dedent(
    ...
    from pypeline import (AUTO_FSM, MAIN, NamedTuple, Reg, hw_func, int16_t,
                          struct, uint1_t)
...
FACTOR_SRC = textwrap.dedent(
    ...
    import os, sys
    sys.path.insert(0, {repo!r})
    from pypeline import (AUTO_FSM, MAIN, NamedTuple, Reg, concat, hw_func,
...
def parse_design(tmpdir, mhz=25.0, name="af_unit_design", source=DESIGN_SRC):
    ...
    its AUTO_FSM tag. A distinct file name per call keeps Python's import
    ...
    tags = AUTO_FSM.GET_TAGS(parser_state)
    assert len(tags) == 1, f"expected one AUTO_FSM tag, got {list(tags)}"
    (key,) = list(tags)
...
def schedule_with(parser_state, key, tag, budget_scale):
    return AUTO_FSM.BUILD_SCHEDULE(parser_state, key, tag, budget_scale)
...
def main():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        cg = AUTO_FSM._Codegen(tag, sched, ps)
        ...
        name, src, _globals = AUTO_FSM.GENERATE_FSM_SOURCE(tag, sched, ps)
        ...
        sched_v2 = AUTO_FSM.BUILD_SCHEDULE(ps_v2, key_v2, tag_v2, 0.9, ctl="v2")
        _n2, src_v2, _g2 = AUTO_FSM.GENERATE_FSM_SOURCE(tag_v2, sched_v2, ps_v2)
        ...
        check(
            AUTO_FSM.ESTIMATE_SCHEDULE_AREA(ps_v2, sched_v2)
            > AUTO_FSM.ESTIMATE_SCHEDULE_AREA(ps, sched),
        ...
        sched_oh = AUTO_FSM.BUILD_SCHEDULE(ps_oh, key_oh, tag_oh, 0.9, ctl="onehot")
        _n3, src_oh, _g3 = AUTO_FSM.GENERATE_FSM_SOURCE(tag_oh, sched_oh, ps_oh)
        check(
        ...
        check(
            AUTO_FSM._register_bit_count(sched_oh) > AUTO_FSM._register_bit_count(sched),
            "the area model charges one-hot for its extra flip-flops",
        )

        print("[automatic control encoding]")
        auto = AUTO_FSM.HARVEST_AUTO_FSM_SCHEDULES(ps, area_sweep=False, ctl="auto")[key]
        ...
        check(
            abs(AUTO_FSM.ESTIMATE_SCHEDULE_AREA(ps, auto) - min(auto_scores.values()))
            < 0.01,
        ...
        check(
            AUTO_FSM.SCHEDULES_EQUAL({key: sched}, {key3: sched3}),
            "the same design schedules identically on re-elaboration",
        )
        name3, src3, _g3 = AUTO_FSM.GENERATE_FSM_SOURCE(tag3, sched3, ps3)
        ...
        check(
            AUTO_FSM.SCHEDULES_EQUAL({key: sched}, {key: copy.deepcopy(sched)}),
            "SCHEDULES_EQUAL matches a deep copy",
        )
        check(
            not AUTO_FSM.SCHEDULES_EQUAL({key: sched}, {key: tight}),
        ...
        check(
            pypeline.AUTO_FSM(tag.func, max_latency=5).max_latency == 5,
            "max_latency is accepted and stored",
        )
        bad = 0
        for value in (1, 0, -3):
            try:
                pypeline.AUTO_FSM(tag.func, max_latency=value)
            except ValueError:
                bad += 1
        try:
            pypeline.AUTO_FSM(tag.func, max_latency="8")
        except TypeError:
            bad += 1
        check(bad == 4, "max_latency below 2, or non-int, is rejected at construction")
        check(
            repr(pypeline.AUTO_FSM(tag.func, max_latency=5))
            != repr(pypeline.AUTO_FSM(tag.func)),
            "the cap is part of the tag's repr (it feeds canonical entity names)",
        )
        unregistered_tag = pypeline.AUTO_FSM(
        ...
        unregistered = AUTO_FSM.BUILD_SCHEDULE(
            ps, unregistered_tag.canonical_key, unregistered_tag, 0.9, ctl="v3"
        )
        _nu, src_u, _gu = AUTO_FSM.GENERATE_FSM_SOURCE(
        ...
        name4, src4, _g4 = AUTO_FSM.GENERATE_FSM_SOURCE(tag, sched, ps)
        check(
            "_af_mux" in src4,
        ...
        check(
            [
                p["n_choices"]
                for p in AUTO_FSM._operand_mux_plan(
                    sched,
                    sched["fu_order"][0],
                    AUTO_FSM.ALLOCATE_REGISTERS(
        ...
        reg_factor, _types_factor = AUTO_FSM.ALLOCATE_REGISTERS(
            sched_factor["nodes"],
            sched_factor["output"],
        ...
        _factor_nodes, factor_ports = AUTO_FSM._operand_mux_plan(
        ...
        _nf, src_factor, _gf = AUTO_FSM.GENERATE_FSM_SOURCE(
        ...
        packs = AUTO_FSM._OUTPUT_SHIFT_PACKS(
        ...
        cross_pack = AUTO_FSM._CROSS_STATE_NODES(
            sched_pack["nodes"], sched_pack["output"], sched_pack["n_states"]
        )
        check(
        ...
        name_pack, src_pack, _gp = AUTO_FSM.GENERATE_FSM_SOURCE(
        ...
        fused_packs = AUTO_FSM._OUTPUT_SHIFT_PACKS(
            sched_fused["nodes"],
            sched_fused["output"],
        ...
        reg_fused, _types_fused = AUTO_FSM.ALLOCATE_REGISTERS(
            sched_fused["nodes"],
            sched_fused["output"],
            sched_fused["n_states"],
        ...
        input_plan = AUTO_FSM._INPUT_STORAGE_PLAN(
            sched_fused["nodes"],
            sched_fused["output"],
            sched_fused["n_states"],
        )
        ...
        _nfp, src_fused, _gfp = AUTO_FSM.GENERATE_FSM_SOURCE(
        ...
        _ns, src_same, _gs = AUTO_FSM.GENERATE_FSM_SOURCE(tag_same, sched_same, ps_same)
        check(
        ...
        plain_regs, _ = AUTO_FSM.ALLOCATE_REGISTERS(bind_nodes, ("node", "c"), 3)
        shared_regs, _ = AUTO_FSM.ALLOCATE_REGISTERS(
        ...
        _nc, src_cross, _gc = AUTO_FSM.GENERATE_FSM_SOURCE(
        ...
        _n5, src5, _g5 = AUTO_FSM.GENERATE_FSM_SOURCE(tag4, unshared, ps4)
        check(
        ...
        base_area = AUTO_FSM.ESTIMATE_SCHEDULE_AREA(ps6, base)
        check(
        ...
        check(
            adder in getattr(ps6, "pypeline_auto_fsm_soft_equiv", {}),
        ...
        gates = AUTO_FSM.BUILD_SCHEDULE(ps6, key6, tag6, 0.9, None, (adder,))
        check(
        ...
        # real yosys cell counts by auto_fsm_area_sweep_compare_test.py; asserted
        # here because that test needs a full synthesis run and this does not.
        all_unshared = AUTO_FSM.BUILD_SCHEDULE(
            ps6, key6, tag6, 0.9, None, (), None, ((adder, 3),)
        )
        check(
            len(all_unshared["fus"]) == 3
            and AUTO_FSM.ESTIMATE_SCHEDULE_AREA(ps6, all_unshared)
            > AUTO_FSM.ESTIMATE_SCHEDULE_AREA(ps6, base),
            f"building three adders is estimated as BIGGER than sharing one "
            f"({AUTO_FSM.ESTIMATE_SCHEDULE_AREA(ps6, base):.0f} shared vs "
            f"{AUTO_FSM.ESTIMATE_SCHEDULE_AREA(ps6, all_unshared):.0f} unshared)",
        )
        check(
            swept["unshared"] == [],
        ...
        swept7 = AUTO_FSM.SWEEP_MIN_AREA_SCHEDULE(ps7, key7, tag7, 0.9)
        check(
            AUTO_FSM.SCHEDULES_EQUAL({key7: swept7}, {key6: swept}),
    ...
    # partial-products array needs (see docs/AUTO_FSM_DESIGN.md and
    ...
    # Real sky130 area (docs/AUTO_FSM_DESIGN.md section 3.8): ESTIMATE_SCHEDULE_AREA's
    ...
    if FAILURES:
        ...
        sys.exit(1)
    print("\nAll AUTO_FSM unit tests passed.")

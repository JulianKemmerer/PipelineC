#!/usr/bin/env python3
"""AUTO_FSM end-to-end scheduling test.

Runs a full pypelinec build (no --comb) on auto_fsm_test.py and asserts the
whole chain the feature depends on:
  ...
  - the pure function's several same-kind operations were FOLDED onto fewer
    ...
    per operation, AUTO_FSM has become an expensive way to add latency)
...
def main():
    ...
    if "AUTO_FSM Pass 1: Scheduling Shared-Resource FSMs" not in out:
        fail("the AUTO_FSM schedule pass never ran")
    ...
    if not m:
        fail("no AUTO_FSM schedule summary printed")
    ...
    # @MAIN is itself named auto_fsm_top, so its own top-level entity files
    # (auto_fsm_top_<hash>.vhd) also match "starts with auto_fsm_, ends with
    # .vhd, not a _top.vhd suffix, no _comb_" -- the same shape as the real
    # FSM blob entity (auto_fsm_<blob_func_name>_<hash>.vhd). Any unrelated
    ...
    # source file's path).
    #
    # Also excluded: auto_fsm_operand_mux_*, the generated operand multiplexer
    # entities (include/pypeline/operators/auto_fsm_mux.py). They are named
    # auto_fsm_* too, but they are what the FSM is BUILT FROM, not the FSM.
    candidates = []
    for root, _dirs, files in os.walk(out_dir):
        for f in files:
            if (
                f.startswith("auto_fsm_")
                and not f.startswith("auto_fsm_top_")
                and not f.startswith("auto_fsm_operand_mux")
                and f.endswith(".vhd")
    ...
    if not candidates:
        fail(f"no generated AUTO_FSM entity VHDL found under {out_dir}")
    if len(candidates) > 1:
        fail(
            f"ambiguous AUTO_FSM entity VHDL candidates under {out_dir}, "
    ...
    print(
        f"AUTO_FSM end-to-end test passed: {key} folded {n_ops} operations onto "

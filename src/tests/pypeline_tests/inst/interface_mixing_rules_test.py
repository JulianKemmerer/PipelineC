...
def legacy(stream_in: chan_intrf.fwd_t, stream_out_ready: uint1_t) -> legacy_t:
...
def test_half_migrated_module_cannot_be_instantiated():
    def wiring(stream_in_if: chan_intrf) -> chan_intrf:
...
class arr_in_t(NamedTuple):
    sinks_if: chan_intrf.fb_t[2]
    stream_out_if: chan_intrf.fwd_t


@hw_func
def arr_in(sinks_if: chan_intrf.fwd_t[2], stream_out_if: chan_intrf.fb_t) -> arr_in_t:
    o: arr_in_t
...
@hw_func
def passthru(stream_in_if: chan_intrf.fwd_t, stream_out_if: chan_intrf.fb_t) -> pass_t:
    o: pass_t
...
def test_unassigned_plain_bundle_field_is_rejected():
    def wiring(stream_in_if: chan_intrf) -> tagged_ports:
...
def plain_reads_wiring(stream_in_if: chan_intrf) -> tagged_ports:
    a = passthru(stream_in_if)
...
@MAIN
def top_plain_reads(stream_in_if: chan_intrf.fwd_t, stream_out_if: chan_intrf.fb_t) -> plain_reads_t:
    return plain_reads(stream_in_if, stream_out_if)


def test_plain_statement_may_read_non_interface_fields():
    sim_reset()
    r = sim_call(top_plain_reads, chan_intrf.fwd_t(data=7, valid=1), chan_intrf.fb_t(ready=1))
    assert int(r.tag) == 14
...
def make_bump(step):
    ...
    @hw_func
    def bump(stream_in_if: chan_intrf.fwd_t, stream_out_if: chan_intrf.fb_t) -> bump_t:
        o: bump_t
...
@MAIN
def top_bump3(stream_in_if: chan_intrf.fwd_t, out_port_if: chan_intrf.fb_t) -> bump3_t:
    return bump3(stream_in_if, out_port_if)


@MAIN
def top_bump9(stream_in_if: chan_intrf.fwd_t, out_port_if: chan_intrf.fb_t) -> bump9_t:
...
def test_factory_instances_are_named_apart():
    ...
    sim_reset()
    r3 = sim_call(top_bump3, chan_intrf.fwd_t(data=10, valid=1), chan_intrf.fb_t(ready=1))
    r9 = sim_call(top_bump9, chan_intrf.fwd_t(data=10, valid=1), chan_intrf.fb_t(ready=1))

...
"""Self-checking make_stream_auto_fsm design: drives the AUTO_FSM valid/ready
stream wrapper (include/pypeline/stream/stream_auto_fsm.py) with a sequence of
...
proving the wrapper elaborates and synthesises with a real AUTO_FSM schedule
installed underneath it), and TWICE in native_vs_vhdl_sim_tests.py: once with
--comb (the AUTO_FSM call site inside the wrapper is still the combinational
...
self_check_auto_fsm_test.py's `o.valid` reaction -- the identical source is
...
def mixed_ops(x: case_t) -> int16_t:
    ...
    self_check_auto_fsm_test.py's mixed_ops, reused here for the same reason:
...
OPS_FSM, OPS_FSM_T = make_stream_auto_fsm(mixed_ops)


@MAIN(25.0)
def self_check_stream_auto_fsm() -> int16_t:
    # Returning the last result gives the design a real output port -- see
    # self_check_auto_fsm_test.py's comment on why this matters for synthesis.
    last: Reg[int16_t]
    ...
    # stream_auto_pipeline.py's ready_reg convention) -- so ready_now is the value
    ...
    if consumed:
        ...
        sim_assert(
            ...
            "make_stream_auto_fsm result did not match the pure function",
        )
        ...
        if case_idx < NUM_CASES - 1:
            sim_print(
                f"self_check_stream_auto_fsm case_idx={case_idx} data={o.stream_out_if.stream.data}",
                debug=True,
            )
        if case_idx == NUM_CASES - 1:
            sim_finish()

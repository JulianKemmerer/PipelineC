...
"""Self-checking make_stream_auto_pipeline design for PIPELINED native sim.

Registered in synth_tests.py as `pypelinec <this file> --sim --run all`
(no --comb): the full build runs first (throughput sweep + AUTO_PIPELINE
.latency pin-and-confirm — make_stream_auto_pipeline reads .latency to size its
FIFO), then the native simulator runs with the discovered latencies emulated
(AUTO_PIPELINE call-site delay lines). The checks are latency-insensitive
...
# (Not the whole stream_auto_pipeline_t: this MAIN takes no arguments -- it drives
# its own stream_in_if from local constants -- so stream_auto_pipeline_t's own
...
@MAIN(50.0)
def self_check_stream_auto_pipeline() -> stream_auto_pipeline.out_fwd_t:
    cycle: Reg[uint16_t]
    ...
    stream_in_if.data = x
    o: stream_auto_pipeline_t = stream_auto_pipeline(

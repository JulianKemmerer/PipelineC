...
"""Self-checking AUTO_FSM design: drives a resource-shared FSM with a sequence of
...
build. That property is worth preserving in any AUTO_FSM testbench.
"""
...
OPS_FSM = AUTO_FSM(mixed_ops)


@MAIN(25.0)
def self_check_auto_fsm() -> int16_t:
    ...
    # run in --comb mode, where the AUTO_FSM call site is still a passthrough
    ...
    if o.valid:
        ...
        sim_assert(o.data == want, "AUTO_FSM result did not match the pure function")
        last = o.data
        busy = 0
        # No debug print on the sim_finish() cycle -- see self_check_counter_test.py.
        if case_idx < NUM_CASES - 1:
            sim_print(f"self_check_auto_fsm case_idx={case_idx} data={o.data}", debug=True)
        if case_idx == NUM_CASES - 1:
            sim_finish()
        case_idx += 1

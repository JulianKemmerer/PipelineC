#!/usr/bin/env python3
"""AUTO_FSM real-sky130-area A/B: does ranking with real cached leaf/register/
multiplexer um2 (docs/AUTO_FSM_DESIGN.md section 3.8) ever measure WORSE than
...
mirrors auto_fsm_area_sweep_compare_test.py's and auto_fsm_min_area_verify_test
...
Method: build qor/divider/auto_fsm.py three ways.
  (default)               real sky130 um2 ranks the area search
  --auto_fsm_abstract_area the abstract per-bit model ranks it instead
  --auto_fsm_no_area_sweep no search at all -- the plain share-everything
                          schedule, common anchor for both rankings above
Then compare each variant's real measured area, and separately check the
register-count fidelity question docs/AUTO_FSM_DESIGN.md section 3.8 raises
explicitly: does AUTO_FSM's OWN allocated storage (live-range registers plus
...
real result is compared with the lowest committed latchup.app AUTO_PIPELINE
...
look cheaper to AUTO_FSM's estimator.
"""
import argparse
import json
import os
...
DESIGN = os.path.join(THIS_DIR, "..", "qor", "divider", "auto_fsm.py")
...
def register_bits(out):
    ...
    if not m:
        fail("no 'register bits: ...' line -- expected DESCRIBE_SCHEDULE to "
             "print AUTO_FSM's own register allocation")
    return int(m.group(1))
...
def main():
    ...
    variants = [
        ...
        ("abstract model", "abstract", ["--auto_fsm_abstract_area"]),
        ("no area sweep (anchor)", "anchor", ["--auto_fsm_no_area_sweep"]),
    ]
    ...
    if real_counts is None:
        fail(
            ...
            "qor/divider/auto_fsm.py declares PART(\"sky130\"); is "
    ...
    if "area model: abstract" not in results["abstract"]["out"]:
        fail(
            "--auto_fsm_abstract_area build's area model line does not say "
            f"abstract -- got {area_model_line(results['abstract']['out'])!r}"
        )

    print()
    print("=== AUTO_FSM real-sky130-area A/B (measured um2, whole design) ===")
    for sub in ("anchor", "abstract", "real"):
        r = results[sub]
        print(
            f"  {r['label']:<28} {r['measured']:>10.1f} um2  "
            f"(AUTO_FSM reg bits {r['reg_bits']:>4}, real seq cells {r['n_seq']:>4})"
        )
    real_vs_abstract = (
    ...
    with open(LATCHUP_MATRIX) as f:
        matrix = json.load(f)
    auto_pipeline_floor = min(
        point["sum_um2"]
        for point in matrix["definitional_area_sum_check"]["results"].values()
    )
    real_vs_auto_pipeline = results["real"]["measured"] / auto_pipeline_floor
    print(
        f"  vs lowest AUTO_PIPELINE divider : {real_vs_auto_pipeline:.3f}x "
        f"({auto_pipeline_floor:.1f} um2 reference)"
    )
    print()
    ...
    if real_vs_abstract > MAX_REGRESSION:
        fail(
            ...
            f"not winning -- see docs/AUTO_FSM_DESIGN.md section 3.8 for the "
            f"known cross-instance-sharing and FF-count caveats before "
            f"assuming a fix is needed in the ranking itself."
        )

    if results["real"]["measured"] >= auto_pipeline_floor:
        fail(
            f"real AUTO_FSM divider area {results['real']['measured']:.1f} um2 "
            f"did not clear the lowest committed AUTO_PIPELINE/latchup.app "
            f"divider area {auto_pipeline_floor:.1f} um2"
        )

    if results["real"]["measured"] > MAX_OPTIMIZED_DIVIDER_AREA_UM2:
        fail(
            f"optimized AUTO_FSM divider measured "
    ...
    # does AUTO_FSM's OWN allocator have anything like the 5.7-5.9x overshoot
    ...
    # real gap here would be a finding for AUTO_FSM's allocator, not evidence
    ...
    if r["n_seq"] > 0:
        ...
        print(
            f"AUTO_FSM register allocator: {r['reg_bits']} bits vs "
            ...
            f"AUTO_FSM's allocator, a different and narrower count, reported "
            f"here for the record rather than gated on."
        )

    print(
        f"AUTO_FSM real-area A/B passed: real sky130 um2 measured "
        f"{real_vs_abstract * 100:+.1f}% vs the abstract model, as built "
        f"(within the {MAX_REGRESSION * 100:.0f}% regression tolerance), and "
        f"{1.0 / real_vs_auto_pipeline:.2f}x smaller than the lowest committed "
        f"AUTO_PIPELINE divider."
    )

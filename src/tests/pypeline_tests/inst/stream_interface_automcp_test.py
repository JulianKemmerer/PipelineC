...
# make_stream_interface_automcp: the valid/ready multi-cycle-path wrapper whose
# cycle count is an AUTOMCP tag. Native sim checks the handshake follows the
...
# automcp_unit_test.py parses it to check elaboration and the cache re-parse.
import sys, os
...
START_LATENCY = 3
FIXED_LATENCY = 2
divider_automcp, divider_automcp_t = make_stream_interface_automcp(
    divider, start_latency=START_LATENCY, max_latency=8
)
divider_fixed_automcp, divider_fixed_automcp_t = make_stream_interface_automcp(
...
@MAIN(100.0)
@hw_func
def automcp_divider_test_fsm() -> uint1_t:
    x: Reg[uint32_t] = 2
    y: Reg[uint32_t] = 1
    ...
    f = divider_automcp(
        ...
        divider_automcp.out_fb_t(ready=1),
    )
    g = divider_fixed_automcp(
        my_struct_intrf.fwd_t(stream=in_stream_if),
        divider_fixed_automcp.out_fb_t(ready=1),
    )
...
def test_start_latency_handshake():
    assert divider_automcp.mcp.latency == START_LATENCY
    _check_handshake(divider_automcp, START_LATENCY)
    print("test_start_latency_handshake PASS")


def test_fixed_latency_handshake():
    assert divider_fixed_automcp.mcp.latency == FIXED_LATENCY
    _check_handshake(divider_fixed_automcp, FIXED_LATENCY)
...
if __name__ == "__main__":
    ...
    print("All stream_interface_automcp tests passed.")

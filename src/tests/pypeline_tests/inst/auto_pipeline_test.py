...
# The raw AUTO_PIPELINE primitive: constructed once, eagerly, at module level
...
TEST_PIPELINE_AP = AUTO_PIPELINE(test_pipeline)
...
print("auto_pipeline_test: TEST_PIPELINE_AP.latency =", TEST_PIPELINE_AP.latency)
assert isinstance(TEST_PIPELINE_AP.latency, int)


# Inline registered-input/registered-output idiom around the AUTO_PIPELINE'd
# call (what _auto_pipeline_with_io_regs builds for library code).
@hw_func
def auto_pipelined_test_pipeline(x: stream_t) -> stream_t:
    in_reg: Reg[stream_t]
...
def valid_ready_pipeline_test(
    ...
    # The auto-pipelined submodule instance, with registered input/output
    pipeline_no_handshake_out: stream_t = auto_pipelined_test_pipeline(
...
@MAIN(50.0)
def auto_pipelined_submodules_main(
    pipeline_in: stream_t, pipeline_out_ready: uint1_t
) -> valid_ready_pipeline_test_t:
    return valid_ready_pipeline_test(pipeline_in, pipeline_out_ready)


def test_auto_pipelined_submodules_sim():
    sim_reset()
    out = sim_call(auto_pipelined_submodules_main, stream_t(data=4, valid=1), 1)
    print(f"test_auto_pipelined_submodules_sim out={out}")
...
if __name__ == "__main__":
    test_auto_pipelined_submodules_sim()

#!/usr/bin/env python3
# AUTOPIPELINE latency constraints end-to-end (full planned sweep, PyRTL):
#  (a) autopipeline_constraints_design.py -- latency=2 and start_latency=1
...
#  (b) autopipeline_max_latency_design.py -- an unreachable goal whose only
...
def harvested(out):
    return {
        key: int(clks)
        for key, clks in re.findall(r"^AUTOPIPELINE (\S+): (\d+) clks$", out, re.M)
    }


def check_fixed_and_start(out_dir):
    rc, out = run(
        "autopipeline_constraints_design.py",
        out_dir,
    ...
    region_lines = re.findall(r"^\[sweep\] AUTOPIPELINE .*clk\(s\) built at", out, re.M)
    ...
    if out_dir:
        ...
        with open(history_path) as f:
            history = json.load(f)
        if "autopipeline_regions" not in json.dumps(history):
            fail(f"no autopipeline_regions entry in {history_path}")


def check_max_latency(out_dir):
    rc, out = run("autopipeline_max_latency_design.py", out_dir)
    if rc == 0:
        fail("unreachable goal behind max_latency=1 did not fail the build")
    if "limited by AUTOPIPELINE latency constraint" not in out:
        fail("sweep did not name the max_latency cap as the limit")
    built = [
        int(n)
        for n in re.findall(
            r"^\[sweep\] AUTOPIPELINE \S+ \(max_latency=1\): (\d+) clk\(s\) built",
            out,
            re.M,
        )
    ]
...
def main():
    ...
    print("All AUTOPIPELINE latency constraint end-to-end tests passed.")

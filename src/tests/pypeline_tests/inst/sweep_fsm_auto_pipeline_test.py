...
# AUTO_PIPELINE region (via _auto_pipeline_with_io_regs). The cut subtree is the
# AUTO_PIPELINE tagged child; cuts descend through the stateful boundary via
...
auto_pipelined_heavy_pipeline, heavy_pipeline_ap = _auto_pipeline_with_io_regs(
...
print(
    "sweep_fsm_auto_pipeline_test: heavy_pipeline AUTO_PIPELINE latency =",
...
@MAIN(40.0)
def sweep_fsm_auto_pipeline_main(
    ...
    o.pipeline_out = auto_pipelined_heavy_pipeline(gated_in)
    return o


def test_sweep_fsm_auto_pipeline_sim():
    sim_reset()
    out = sim_call(sweep_fsm_auto_pipeline_main, stream_t(data=4, valid=1), 1)
    print(f"test_sweep_fsm_auto_pipeline_sim out={out}")


if __name__ == "__main__":
    test_sweep_fsm_auto_pipeline_sim()
    print("All sweep fsm auto-pipeline tests passed.")

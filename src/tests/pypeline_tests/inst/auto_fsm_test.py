...
"""AUTO_FSM basics: a pure function with several same-kind operations, wrapped
...
Registered three ways, each proving something different:
  native_sim  plain `python3 auto_fsm_test.py` -- the tag behaves as an
  ...
  synth       driven by the auto_fsm_latency_test wrapper, which runs the full
...
def blob(x: blob_in_t) -> int16_t:
    ...
    what AUTO_FSM folds onto shared hardware, plus enough variety to exercise
...
BLOB_FSM = AUTO_FSM(blob)


@MAIN(25.0)
def auto_fsm_top(start: uint1_t, x: blob_in_t) -> int16_t:
...
if __name__ == "__main__":
    ...
    assert BLOB_FSM.latency == 0, "unscheduled AUTO_FSM should report latency 0"

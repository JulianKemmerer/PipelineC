...
"""AUTO_PIPELINE(func, latency=N) in plain native simulation.

A fixed latency is a functional contract, so -- unlike an unconstrained
AUTO_PIPELINE, which is a zero-latency passthrough until a build installs its
...
FIXED3 = AUTO_PIPELINE(plus_seven, latency=3)
FIXED0 = AUTO_PIPELINE(plus_seven, latency=0)
STARTED = AUTO_PIPELINE(plus_seven, start_latency=2, max_latency=4)
CAPPED = AUTO_PIPELINE(plus_seven, max_latency=1)
...
def two_fixed_same_line(x: uint16_t) -> uint16_t:
    # Two different AUTO_PIPELINE objects over one func: each call site keeps
...
if __name__ == "__main__":
    ...
    print("All AUTO_PIPELINE fixed latency native sim tests passed.")

...
"""Design for the AUTO_FSM minimum-area verification test
(auto_fsm_min_area_verify_test.py): the OPEN showcase.
...
and unlike a multiply it has a soft equivalent AUTO_FSM can descend into: a
...
builds (--auto_fsm_open / --auto_fsm_unshare) are the alternatives the search
passed over.
...
@MAIN(1.0)
def auto_fsm_div_share_top(start: uint1_t, x: div_in_t) -> uint8_t:

"""Operand multiplexers for AUTO_FSM's shared functional units.
...
stops sharing from paying -- so AUTO_FSM needs the mux to be (a) built the way
...
FSM source:
  ...
  * ARRAY-INDEXED, NOT A PRIORITY CHAIN. `choices[sel]` elaborates to the
    ...
    the regime AUTO_FSM's area sweep pushes into.
...
# callable identity for the whole process: AUTO_FSM reverse-looks-up the entity
...
def make_operand_mux(t, n: int):
    ...
    n must be >= 2 -- a single-user port needs no mux at all, and AUTO_FSM wires
    ...
    @hw_func
    def auto_fsm_operand_mux(sel: sel_t, choices: choices_t) -> t:
        return choices[sel]

    _MUX_CACHE[key] = auto_fsm_operand_mux
    return auto_fsm_operand_mux

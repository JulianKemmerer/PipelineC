...
def make_stream_auto_fsm(func, max_latency=None):
    """Wraps a pure combinational hardware function in an AUTO_FSM instance plus a
    registered-output valid/ready handshake, exposing it as a stream interface --
    the AUTO_FSM equivalent of make_stream_auto_pipeline (AUTO_PIPELINE) and
    make_stream_multi_cycle (MULTI_CYCLE[...]).

    AUTO_FSM's own call-site contract has no backpressure: a `valid` pulse
    ...
    space requests at least `.latency` cycles apart (see the AUTO_FSM class
    ...
    The wrapper disables AUTO_FSM's own optional result bank because the
    ...
    Returns (func_auto_fsm, func_auto_fsm_t):
        func_auto_fsm(stream_in_if: in_intrf.fwd_t, stream_out_if: out_intrf.fb_t) -> func_auto_fsm_t
        func_auto_fsm_t fields: .stream_in_if (in_intrf.fb_t), .stream_out_if (out_intrf.fwd_t)
    ...
    raw AUTO_FSM FSM, spent latching its pulse into the holding register. The
    ...
    AUTO_PIPELINE/AUTO_FSM call sites elsewhere (see docs/AUTO_FSM_DESIGN.md and
    the "CONSTRUCTION TIMING MATTERS" note on AUTO_FSM itself).
    """
    if not is_hw_func(func):
        raise TypeError(
            f"make_stream_auto_fsm(func, ...): {func.__qualname__!r} must be "
            f"@hw_func-decorated before being passed in"
        )
    # The holding register below is already the stream's output boundary.
    # Asking raw AUTO_FSM for another full-width output register would only
    # duplicate storage and add muxing; expose its final-state value directly.
    fsm = AUTO_FSM(func, max_latency=max_latency, register_output=False)

    in_intrf = make_stream_interface(fsm.in_type)
    out_intrf = make_stream_interface(fsm.out_type)

    @struct
    class func_auto_fsm_t(NamedTuple):
    ...
    def func_auto_fsm(
        ...
        # make_stream_multi_cycle trick): clearing result_valid here is visible
    ...
    func_auto_fsm.fsm = fsm
    func_auto_fsm.latency = fsm.latency + 1
    func_auto_fsm.in_intrf = in_intrf
    func_auto_fsm.out_intrf = out_intrf
    func_auto_fsm.in_fwd_t = in_intrf.fwd_t
    func_auto_fsm.in_fb_t = in_intrf.fb_t
    func_auto_fsm.out_fwd_t = out_intrf.fwd_t
    func_auto_fsm.out_fb_t = out_intrf.fb_t
    return func_auto_fsm, func_auto_fsm_t

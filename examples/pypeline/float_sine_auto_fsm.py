...
"""AUTO_FSM example: the sine polynomial from float_sine.py, folded onto shared
...
AUTO_FSM schedules exactly that: one float64 multiplier and one float64 adder,
reused across as many states as the clock goal requires.

    $ pypelinec examples/pypeline/float_sine_auto_fsm.py

Compare the `AUTO_FSM ...: N ops -> M shared unit(s)` line and the yosys cell
...
about the size of the combinational blob, not about AUTO_FSM. The polynomial is
where the folding win is anyway.

AUTO_FSM takes exactly one argument, so the polynomial's four inputs are bundled
...
# enormous under any model -- it is indivisible as far as AUTO_FSM is concerned
...
class poly_in_t(NamedTuple):
    """All of sinf_poly's inputs in one struct: AUTO_FSM wraps single-argument
...
POLY_FSM = AUTO_FSM(sinf_poly)


@MAIN(CLOCK_MHZ)
def float_sine_auto_fsm(start: uint1_t, i: poly_in_t) -> float32_t:

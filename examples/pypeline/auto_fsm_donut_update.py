...
"""AUTO_FSM example: the donut demo's once-per-frame rotation math, implemented
as a resource-shared FSM instead of a slab of parallel combinational logic.

This is the textbook case for AUTO_FSM. In examples/pypeline/vga_donut.py the
...
Wrapped in AUTO_FSM, the tool schedules the same math onto a handful of shared
...
schedules them against the clock goal, and reports what it did.
    ...
    $ pypelinec examples/pypeline/auto_fsm_donut_update.py

Look for the `AUTO_FSM ...: N ops -> M shared unit(s)` line in the build log,
and compare against a `--comb` build of the same file (where the AUTO_FSM call
...
math so the example is about AUTO_FSM, and vga_donut.py stays the reference
...
def next_orientation(s: orientation_t) -> orientation_t:
    ...
    constant, i.e. free rewiring), which is exactly the shape AUTO_FSM folds
...
UPDATE_FSM = AUTO_FSM(next_orientation)


@MAIN(CLOCK_MHZ)
def auto_fsm_donut_update() -> orientation_t:

...
fd_lut_update_t insert_fd(fd_t host_fildes, uint1_t is_bram, fd_lut_t fd_lut)
{
...
posix_sys_to_proc_t main_sys_to_proc;
...
posix_proc_to_sys_t main_proc_to_sys;
...
void fosix()
{
  ...
  posix_sys_to_proc_t sys_to_proc_host;
  WIRE_READ(posix_sys_to_proc_t, sys_to_proc_host, host_sys_to_proc)
  // Read sys_to_proc_bram output from bram
  posix_sys_to_proc_t sys_to_proc_bram;
  WIRE_READ(posix_sys_to_proc_t, sys_to_proc_bram, bram_sys_to_proc)
  // Read proc_to_sys output from main process
  posix_proc_to_sys_t proc_to_sys_main;
  WIRE_READ(posix_proc_to_sys_t, proc_to_sys_main, main_proc_to_sys)
  
  // Outputs
  // Default outputs so each state is easier to write
  posix_proc_to_sys_t proc_to_sys_host = POSIX_PROC_TO_SYS_T_NULL();
  posix_proc_to_sys_t proc_to_sys_bram = POSIX_PROC_TO_SYS_T_NULL();
  posix_sys_to_proc_t sys_to_proc_main = POSIX_SYS_TO_PROC_T_NULL();
  //////////////////////////////////////////////////////////////////////
  ...
  else if(fosix_state==WAIT_REQ)
  {
    ...
    else if(proc_to_sys_main.sys_write.req.valid)
    {
      ...
      fd_t local_fd = proc_to_sys_main.sys_write.req.fildes;
    ...
    else if(proc_to_sys_main.sys_read.req.valid)
    {
      ...
      // Before connecting to sys proc_to_sys device busses
      // Saving a copy of local fd
      fd_t local_fd = proc_to_sys_main.sys_read.req.fildes;
    ...
    else if(proc_to_sys_main.sys_close.req.valid)
    {
      ...
      // Before connecting to sys proc_to_sys device busses
      // Saving a copy of local fd
      fd_t local_fd = proc_to_sys_main.sys_close.req.fildes;
  ...
  */
  ...
  WIRE_WRITE(posix_proc_to_sys_t, host_proc_to_sys, proc_to_sys_host)
  // Write proc_to_sys_bram output into bram
  WIRE_WRITE(posix_proc_to_sys_t, bram_proc_to_sys, proc_to_sys_bram)
  // Write sys_to_proc_main output into main
  WIRE_WRITE(posix_sys_to_proc_t, main_sys_to_proc, sys_to_proc_main)
}

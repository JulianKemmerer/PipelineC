...
bleghs needs lots of find replace for reqresp name change

posix_sys_to_proc_t do_syscall_get_resp(posix_proc_to_sys_t req, uart_msg_t msg)
{
  posix_sys_to_proc_t resp = POSIX_SYS_TO_PROC_T_NULL();
...
int main(int argc, char **argv) 
{
  ...
  while(!exit)
  {
    ...
    uart_msg_t read_msg = msg_read();
    printf("FOSIX: Read DMA msg...\n");
    
    // Convert to sys request struct
    posix_proc_to_sys_t request = msg_to_request(read_msg);
    
    // Do the requested syscall and form response
    posix_sys_to_proc_t response = do_syscall_get_resp(request, read_msg);
    
    // Convert to dma message
    msg_msg_s write_msg = response_to_dma(response);
    
    // Write response dma msg 
    if(write_msg.valid)
    {
      printf("FOSIX: Writing DMA msg...\n");
      msg_write(write_msg.data);

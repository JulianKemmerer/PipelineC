...
for i, x_int in enumerate(x_ints):
    ...
    y_text += (
        "Y_VALUES[" + str(lut_addr_bits) + "].qmn = " + str(hex(y_ints[i])) + ";\n"
    )
    m_text += (
        "M_VALUES[" + str(lut_addr_bits) + "].qmn = " + str(hex(m_ints[i])) + ";\n"
    )
...
print(
    "uint"
    + str(LOG2_NUM_ENTRIES)
    + "_t lut_addr = int"
    + str(INPUT_FIXED_BITWIDTH)
    + "_"
    + str(INPUT_FIXED_BITWIDTH - 1)
    + "_"
    + str(INPUT_FIXED_BITWIDTH - LOG2_NUM_ENTRIES)
    + "(x.qmn);"
)
print("// And interpolation bits from lsbs")
print(
    "uint"
    + str(INTERP_FIXED_BITS)
    + "_t interp_point = int"
    + str(INPUT_FIXED_BITWIDTH)
    + "_"
    + str(INTERP_FIXED_BITS - 1)
    + "_0(x.qmn);"
)
print("// Generated lookup values:")
print(
    "q" + str(OUTPUT_M) + "_" + str(OUTPUT_N) + "_t Y_VALUES[" + str(NUM_ENTRIES) + "];"
)
print(y_text)
...
print(
    "int"
    + str(INPUT_FIXED_BITWIDTH + M_scaled_FIXED_BITWIDTH)
    + "_t temp = dxi.qmn * m.qmn;"
)
print(
    "int"
    + str(INPUT_FIXED_BITWIDTH + M_scaled_FIXED_BITWIDTH)
    + "_t temp_rounded = temp + (1 << ("
    + str(INPUT_N)
    + " - 1));"
)
print(
    """// Shift right by """
    + str(INPUT_N)
    + " for normal Q mult, then shift left by "
    + str(m_right_shift_scale)
    + """ to account for slope scaling"""
)

...
uint1_t uart_tx_bit_buffer_valid;
// Output type
...
uart_tx_mac_o_t uart_tx_mac(uart_mac_s word_in)
{
  ...
  // Ready for an incoming incoming word to send
  // if dont have valid word already (i.e. buffer empty)
  output.word_in_ready = !uart_tx_bit_buffer_valid;
  ...
  if(output.word_in_ready)
  {
    ...
    {
      uart_tx_bit_buffer[i] = word_in.data >> i;
    }
    uart_tx_bit_buffer_valid = word_in.valid;
  }
  
  // State machine for transmitting
  if(uart_tx_mac_state==IDLE)
  {
    // Wait for valid bits in buffer
    if(uart_tx_bit_buffer_valid)
    {
  ...
  else if(uart_tx_mac_state==TRANSMIT)
  {
    ...
    if(uart_tx_clk_counter >= UART_CLKS_PER_BIT)
    {
      ...
      if(uart_tx_bit_counter==UART_WORD_BITS)
      {
        ...
        uart_tx_clk_counter = 0;
        // Done with bits in buffer now
        uart_tx_bit_buffer_valid = 0;
      }
    }
  }
...
sys_clk_main_outputs_t sys_clk_main(sys_clk_main_inputs_t inputs)
{
  ...
  uart_tx_mac_o_t uart_tx_mac_output = uart_tx_mac(rx_word);

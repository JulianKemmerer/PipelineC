...
#include "clock_crossing/the_slow_clock.h"
...
void clk_gen()
{
  ...
  WIRE_WRITE(uint1_t, the_slow_clock, clock)
  // Toggle logic
...
uint32_t main()
{
  ...
  static uint32_t counter = 0;
  uint32_t rv = counter;
  // Temp use clock as increment for test
  uint1_t slow_clock;
  WIRE_READ(uint1_t, slow_clock, the_slow_clock)
  counter += slow_clock;
  //counter += 1;

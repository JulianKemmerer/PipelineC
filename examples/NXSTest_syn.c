...
#pragma MAIN_MHZ DoNXSTest 400.0
uint64_t DoNXSTest(uint64_t State[27], uint64_t Key[17])
{

	SkeinRoundTest_t srt = SkeinRoundTest0(State, Key); // T inside func
  State = srt.State;
  Key = srt.Key;
	
	ARRAY_COPY(Key, State, 16)  //memcpy(Key, State, 128); // Key[127:0] = State[127:0]
	ARRAY_SET(State, 0x00, 16) //memset(State, 0x00, 128);
	Key[16] = SKEIN_KS_PARITY;

  uint32_t i;
	for(i = 0; i < 16; i+=1) Key[16] ^= Key[i];
	
	SkeinRoundTest_t srt = SkeinRoundTest1(State, Key); // T inside func
  State = srt.State;
  Key = srt.Key;
  ...
  uint64_t KeccakState[25];
	ARRAY_SET(KeccakState, 0x00, 25) // memset(KeccakState, 0x00, 200);
	ARRAY_COPY(KeccakState, State, 9) // memcpy(KeccakState, State, 72);

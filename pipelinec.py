...
RISCV CPU and stuff?
  Put a risc(v? CHIPS2.0?) core along side accelerator in pipelinec
  host OS on FOSIX NoC?
SDR project? Receive FM radio audio?
Victor's RT
  Try ECP5 flow again?
    Too many FFs still for ECP5 720p 89076/83640
    480p has been running nextpnr for days? never finished...
  Try verilating whole pipeline?
Victor's other secret thing
Mandelbrot demo with frame buffer FSM style and pipelined iterations?
...
Make GoL demo faster, 
  Multi thread GoL, try easy split in two from both sides of line?
    Computed line next state can be done in arbitrary pieces line buffer
    Just AND together done wires from n threads
    Might have more a little read bandwidth but doesnt seem like 2x needed

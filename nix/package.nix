  ...
  installPhase = ''
    ...
    for compiler in pipelinec pypelinec; do
      makeWrapper ${python}/bin/python3 "$out/bin/$compiler" \
        ...
        --run "if [ ! -d .pypelinec_cache ]; then ${coreutils}/bin/cp -r '$out/lib/pypelinec/cache' .pypelinec_cache && ${coreutils}/bin/chmod -R u+w .pypelinec_cache; fi" \
        --run 'export PYPELINEC_CACHE_DIR="$PWD/.pypelinec_cache/"'
    done
